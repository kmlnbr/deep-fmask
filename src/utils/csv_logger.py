...
CSV_HEADERS['overall_statistics.csv']= ['Epoch', 'Train_Loss','Train_Accuracy', 'Valid_Loss','Valid_Accuracy','Valid_MIOU']
...
def print_val_csv_metrics(row_number, output_data_path):
    l = '{}\t{:>9} \t{:>9} \t{:>9} \t{:>9} \t{:>9}'.format('metrics'.ljust(15),*classname)
    print(l)
    logger.info(l)
    for filename in list(CSV_HEADERS.keys()):
        ...
        l='{}\t{:>9} \t{:>9} \t{:>9} \t{:>9} \t{:>9} '.format(metric_name.ljust(15),*csv_row)
        print(l)
        logger.info(l)
...
def print_pred_csv_metrics(output_data_path,filename):
    ...
    for i in range(1,5):
        ...
        l = '{}\t{:>9} \t{:>9} \t{:>9} \t{:>9} \t{:>9} '.format(metric_name.ljust(15),*csv_row[1:])
        print(l)
        logger.info(l)
...
def make_overall_statistics_csv(train_metrics, valid_metrics,class_metrics_dict, epoch,log_path):
    ...
    for metric in ['Precision', 'Recall', 'f1', 'iou']:
        ...
        add_to_csv(row_entry, log_path,filename='valid_{}_class.csv'.format(metric.lower()))
